  ...
  Future<String> signInUser(String email, String password) async {
    ...
    try {
      ...
      if (isUser == true) {
        ...
        var deneme =
            await _firestore.collection("user").doc(user.user.uid).get();
  ...
  Future<String> signInAdmin(String email, String password) async {
    try {
      ...
      if (isAdmin == true) {
        ...
        var deneme =
            await _firestore.collection("admin").doc(user.user.uid).get();
...
//get rezervation
  Future<List<RezervationModel>> getRezervationByUser() async {
    ...
    var rezervasyonlar = await _firestore
        .collection("user")
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var d = await _firestore
          ...
          .where("userId", isEqualTo: "8lUaVak3E0QoWhv761INrKyxE7h2")
          .get();
      var t = d.docs.last;
...
//bu fonksiyonu sayfanın constructurun da çağoırmak istiyorum
// hangi sayfada

//Kullanıcnın Favori Kafelerinin Id sini Çekme Fonksiyonu
  Future<void> getFavoriteCafes() async {
    print(CurrentUserId);
    ...
    var data = temp.docs;
    for (var i = 0; i < data.length; i++) {
      FavoriteCafesList.add(data[i].id);
      await getFavoriteCafeInfo(data[i]["CafeId"]);
    }
  }

  //Kullanıcının Favori Kafelerine Göre Kafe Bilgilerini Çekme
  Future<void> getFavoriteCafeInfo(String CafeId) async {
    CafeModel tempmodel = CafeModel();
    var document = await _firestore.collection("cafe").doc(CafeId).get();
    print(document.data().isEmpty);
    tempmodel.name = document.get("name");
    tempmodel.picture = document.get("picture");
    tempmodel.pictureUrl = document.get("pictureUrl");
    ModelFavCafe.add(tempmodel);
  }
  ...
  Future<User> createAdmin(
    ...
    await _firestore.collection("usersAndAdmins").doc(user.user.uid).set({
      'userName': name,
      'email': email,
      'role': "admin",
      'uid': user.user.uid
  ...
  void addCafe(
    ...
    firebase_storage.Reference firebaseStorageRef = firebase_storage
        ...
        .ref()
        .child('cafeImages/$fileName');

    firebase_storage.UploadTask uploadTask =
        firebaseStorageRef.putFile(_imageFile);

    firebase_storage.Reference firebaseStorageRef1 = firebase_storage
        .FirebaseStorage.instance
        .ref()
        .child('cafeMenuPdf/$pdfName');
...
//rezervasyon ekle
  void addRezervation(String peoplecounter, String cafeId, String note,
      String date, String cafeName) async {
    FirebaseFirestore.instance
        .collection("cafe")
        .doc(cafeId)
        .collection("rezervasyonlar")
  ...
  void updateRezervation(String peoplecounter, String cafeId, String note,
      String date, String caferezervationId, String userrezervationId) async {
    FirebaseFirestore.instance
        .collection("cafe")
        .doc(cafeId)
        .collection("rezervasyonlar")
        .doc(caferezervationId)
        .update({
      'name': userName,
      'userId': _auth.currentUser.uid,
      'date': date,
